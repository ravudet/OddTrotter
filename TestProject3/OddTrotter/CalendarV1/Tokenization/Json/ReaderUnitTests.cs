...
""";
        ...
        public static async Task StaticOnly(Stream stream)
        {
            ...
            var whitespaceReader = await reader.Move(context).ConfigureAwait(false);
            (context, var valueReader) = await whitespaceReader.Move(context).ConfigureAwait(false);
            //var valueReader = await whitespaceReader.Move21(context).ConfigureAwait(false);
            var valueToken = await valueReader.Move(context).ConfigureAwait(false);
            //var valueToken = await valueReader.Move31(context).ConfigureAwait(false);
            Assert.IsTrue(valueToken.TryObject(out var @object));
            var objectStart = await @object.Move(context).ConfigureAwait(false);
            var whitespacereader2 = await objectStart.Move(context).ConfigureAwait(false);
            (context, var membersReader) = await whitespacereader2.Move(context).ConfigureAwait(false);
            //var membersReader = await whitespacereader2.Move21(context).ConfigureAwait(false);
            var membersToken = await membersReader.Move(context).ConfigureAwait(false);
            ...
            // true
            var memberReader = await firstMemberReader.Move(context).ConfigureAwait(false);
            var stringReader = await memberReader.Move(context).ConfigureAwait(false);
            var stringDelimiterReader = await stringReader.Move(context).ConfigureAwait(false);
            var charsReader = await stringDelimiterReader.Move(context).ConfigureAwait(false);
            (context, var stringDelimiterReader2) = await charsReader.Move(context).ConfigureAwait(false);
            //var stringDelimiterReader2 = await charsReader.Move21(context).ConfigureAwait(false);
            var whitespaceReader3 = await stringDelimiterReader2.Move(context).ConfigureAwait(false);
            (context, var colonReader) = await whitespaceReader3.Move(context).ConfigureAwait(false);
            //var colonReader = await whitespaceReader3.Move21(context).ConfigureAwait(false);
            var whitespaceReader4 = await colonReader.Move(context).ConfigureAwait(false);
            (context, var valueReader2) = await whitespaceReader4.Move(context).ConfigureAwait(false);
            //var valueReader2 = await whitespaceReader4.Move21(context).ConfigureAwait(false);
            var valueToken2 = await valueReader2.Move(context).ConfigureAwait(false);
            Assert.IsTrue(valueToken2.TryTrue(out var @true));
            (context, var subsequentMembersReader) = await @true.Move(context).ConfigureAwait(false);
            var subsequentMembersToken = await subsequentMembersReader.Move(context).ConfigureAwait(false);
            Assert.IsTrue(subsequentMembersToken.TryMore(out var subsequentMemberReader));
            var commaReader = await subsequentMemberReader.Move(context).ConfigureAwait(false);
            var whitespaceReader5 = await commaReader.Move(context).ConfigureAwait(false);

            // false
            (context, var memberReader2) = await whitespaceReader5.Move(context).ConfigureAwait(false);
            //var memberReader2 = await whitespaceReader5.Move21(context).ConfigureAwait(false);
            var stringReader2 = await memberReader2.Move(context).ConfigureAwait(false);
            var stringDelimterReader3 = await stringReader2.Move(context).ConfigureAwait(false);
            var charsReader2 = await stringDelimterReader3.Move(context).ConfigureAwait(false);
            (context, var stringDelimiterReader4) = await charsReader2.Move(context).ConfigureAwait(false);
            var whitespace6 = await stringDelimiterReader4.Move(context).ConfigureAwait(false);
            (context, var colon2) = await whitespace6.Move(context).ConfigureAwait(false);
            //var colon2 = await whitespace6.Move21(context).ConfigureAwait(false);
            var whitespace7 = await colon2.Move(context).ConfigureAwait(false);
            (context, var value3) = await whitespace7.Move(context).ConfigureAwait(false);
            //var value3 = await whitespace7.Move21(context).ConfigureAwait(false);
            var valueToken3 = await value3.Move(context).ConfigureAwait(false);
            Assert.IsTrue(valueToken3.TryFalse(out var @false));
            (context, var subsequentMembers2) = await @false.Move(context).ConfigureAwait(false);
            var subsequentMembersToken2 = await subsequentMembers2.Move(context).ConfigureAwait(false);
            Assert.IsTrue(subsequentMembersToken2.TryMore(out var subsequentMember2));

            // 1234
            var comma2 = await subsequentMember2.Move(context).ConfigureAwait(false);
            var whitespace8 = await comma2.Move(context).ConfigureAwait(false);
            (context, var member3) = await whitespace8.Move(context).ConfigureAwait(false);
            //var member3 = await whitespace8.Move21(context).ConfigureAwait(false);
            var string3 = await member3.Move(context).ConfigureAwait(false);
            var stringDelimiter5 = await string3.Move(context).ConfigureAwait(false);
            var chars3 = await stringDelimiter5.Move(context).ConfigureAwait(false);
            (context, var stringDelimiter6) = await chars3.Move(context).ConfigureAwait(false);
            var whitespace9 = await stringDelimiter6.Move(context).ConfigureAwait(false);
            (context, var colon3) = await whitespace9.Move(context).ConfigureAwait(false);
            //var colon3 = await whitespace9.Move21(context).ConfigureAwait(false);
            var whitespace10 = await colon3.Move(context).ConfigureAwait(false);
            (context, var value4) = await whitespace10.Move(context).ConfigureAwait(false);
            //var value4 = await whitespace10.Move21(context).ConfigureAwait(false);
            var valueToken4 = await value4.Move(context).ConfigureAwait(false);
            Assert.IsTrue(valueToken4.TryNumber(out var number));
            var sign = await number.Move(context).ConfigureAwait(false);
            var @int = await sign.Move(context).ConfigureAwait(false);
            (context, var frac) = await @int.Move(context).ConfigureAwait(false);
            var fracToken = await frac.Move(context).ConfigureAwait(false);
            Assert.IsTrue(fracToken.TryAbsent(out var exp));
            var expToken = await exp.Move(context).ConfigureAwait(false);
            Assert.IsTrue(expToken.TryAbsent(out var subsequentMembers3));
            var subsequentMembersToken3 = await subsequentMembers3.Move(context).ConfigureAwait(false);
            Assert.IsTrue(subsequentMembersToken3.TryMore(out var subsequentMember3));

            // asdf
            var comma3 = await subsequentMember3.Move(context).ConfigureAwait(false);
            var whitespace11 = await comma3.Move(context).ConfigureAwait(false);
            (context, var member4) = await whitespace11.Move(context).ConfigureAwait(false);
            //var member4 = await whitespace11.Move21(context).ConfigureAwait(false);
            var string4 = await member4.Move(context).ConfigureAwait(false);
            var stringDelimiter7 = await string4.Move(context).ConfigureAwait(false);
            var chars4 = await stringDelimiter7.Move(context).ConfigureAwait(false);
            (context, var stringDelimiter8) = await chars4.Move(context).ConfigureAwait(false);
            var whitespace12 = await stringDelimiter8.Move(context).ConfigureAwait(false);
            (context, var colon4) = await whitespace12.Move(context).ConfigureAwait(false);
            //var colon4 = await whitespace12.Move21(context).ConfigureAwait(false);
            var whitespace13 = await colon4.Move(context).ConfigureAwait(false);
            (context, var value5) = await whitespace13.Move(context).ConfigureAwait(false);
            var valueToken5 = await value5.Move(context).ConfigureAwait(false);
            Assert.IsTrue(valueToken5.TryString(out var @string5));
            var stringDelimiter9 = await string5.Move(context).ConfigureAwait(false);
            var chars5 = await stringDelimiter9.Move(context).ConfigureAwait(false);
            (context, var stringDelimiter10) = await chars5.Move(context).ConfigureAwait(false);
            var subsequentMembers4 = await stringDelimiter10.Move(context).ConfigureAwait(false);
            var subsequentMembersToken4 = await subsequentMembers4.Move(context).ConfigureAwait(false);
            Assert.IsTrue(subsequentMembersToken4.TryMore(out var subsequentMember4));

            // null
            var comma4 = await subsequentMember4.Move(context).ConfigureAwait(false);
            var whitespace14 = await comma4.Move(context).ConfigureAwait(false);
            (context, var member5) = await whitespace14.Move(context).ConfigureAwait(false);
            var string6 = await member5.Move(context).ConfigureAwait(false);
            var stringDelimiter11 = await string6.Move(context).ConfigureAwait(false);
            var chars6 = await stringDelimiter11.Move(context).ConfigureAwait(false);
            (context, var stringDelimiter12) = await chars6.Move(context).ConfigureAwait(false);
            var whitespace15 = await stringDelimiter12.Move(context).ConfigureAwait(false);
            (context, var colon5) = await whitespace15.Move(context).ConfigureAwait(false);
            var whitespace16 = await colon5.Move(context).ConfigureAwait(false);
            (context, var value6) = await whitespace16.Move(context).ConfigureAwait(false);
            var valueToken6 = await value6.Move(context).ConfigureAwait(false);
            Assert.IsTrue(valueToken6.TryNull(out var @null));
            (context, var subsequentMembers5) = await @null.Move(context).ConfigureAwait(false);
            var subsequentMembersToken5 = await subsequentMembers5.Move(context).ConfigureAwait(false);
            Assert.IsTrue(subsequentMembersToken5.TryMore(out var subsequentMember5));


            // object
            var comma5 = await subsequentMember5.Move(context).ConfigureAwait(false);
            var whitespace17 = await comma5.Move(context).ConfigureAwait(false);
            //var member6 = await whitespace17.AsReader.MoveInternal3(whitespace17.Factory).ConfigureAwait(false);
            (context, var member6) = await whitespace17.Move(context).ConfigureAwait(false);
            var string7 = await member6.Move(context).ConfigureAwait(false);
            var stringDelimiter13 = await string7.Move(context).ConfigureAwait(false);
            var chars7 = await stringDelimiter13.Move(context).ConfigureAwait(false);
            (context, var stringDelimiter14) = await chars7.Move(context).ConfigureAwait(false);
            var whitespace18 = await stringDelimiter14.Move(context).ConfigureAwait(false);
            //var colon6 = await whitespace18.AsReader.MoveInternal3(whitespace18.Factory).ConfigureAwait(false);
            (context, var colon6) = await whitespace18.Move(context).ConfigureAwait(false);
            var whitespace19 = await colon6.Move(context).ConfigureAwait(false);
            //var value7 = await whitespace19.AsReader.MoveInternal3(whitespace19.Factory).ConfigureAwait(false);
            (context, var value7) = await whitespace19.Move(context).ConfigureAwait(false);
            var valueToken7 = await value7.Move(context).ConfigureAwait(false);
            Assert.IsTrue(valueToken7.TryObject(out var object2));
            var objectStart2 = await object2.Move(context).ConfigureAwait(false);
            var whitespace20 = await objectStart2.Move(context).ConfigureAwait(false);
            //var nestedMembers = await whitespace20.AsReader.MoveInternal3(whitespace20.Factory).ConfigureAwait(false);
            (context, var nestedMembers) = await whitespace20.Move(context).ConfigureAwait(false);
            var nestedMembersToken = await nestedMembers.Move(context).ConfigureAwait(false);
            Assert.IsTrue(nestedMembersToken.TrySome(out var nestedFirstMember));

            Json5.SubsequentMembersReader<Json5.WhitespaceReader<Json5.ObjectEndReader<Json5.WhitespaceReader<Nothing>>>> subsequentMembers6;
            {
                // true
                var nestedMember = await nestedFirstMember.Move(context).ConfigureAwait(false);
                var nestedString = await nestedMember.Move(context).ConfigureAwait(false);
                var nestedStringDelimiter = await nestedString.Move(context).ConfigureAwait(false);
                var nestedChars = await nestedStringDelimiter.Move(context).ConfigureAwait(false);
                (context, var nestedStringDeliminter2) = await nestedChars.Move(context).ConfigureAwait(false);
                var nestedWhitespace = await nestedStringDeliminter2.Move(context).ConfigureAwait(false);
                //var nestedColon = await nestedWhitespace.AsReader.MoveInternal3(nestedWhitespace.Factory).ConfigureAwait(false);
                (context, var nestedColon) = await nestedWhitespace.Move(context).ConfigureAwait(false);
                var nestedWhispace2 = await nestedColon.Move(context).ConfigureAwait(false);
                //var nestedValue = await nestedWhispace2.AsReader.MoveInternal3(nestedWhispace2.Factory).ConfigureAwait(false);
                (context, var nestedValue) = await nestedWhispace2.Move(context).ConfigureAwait(false);
                var nestedValueToken = await nestedValue.Move(context).ConfigureAwait(false);
                Assert.IsTrue(nestedValueToken.TryTrue(out var nestedTrue));
                (context, var nestedSubsequentMembersReader) = await nestedTrue.Move(context).ConfigureAwait(false);
                var nestedSubsequentMembersToken = await nestedSubsequentMembersReader.Move(context).ConfigureAwait(false);
                Assert.IsTrue(nestedSubsequentMembersToken.TryMore(out var nestedsubsequentMemberReader));

                // false
                var nestedcommaReader = await nestedsubsequentMemberReader.Move(context).ConfigureAwait(false);
                var nestedwhitespaceReader5 = await nestedcommaReader.Move(context).ConfigureAwait(false);
                ...
                (context, var nestedmemberReader2) = await nestedwhitespaceReader5.Move(context).ConfigureAwait(false);
                var nestedstringReader2 = await nestedmemberReader2.Move(context).ConfigureAwait(false);
                var nestedstringDelimiterReader3 = await nestedstringReader2.Move(context).ConfigureAwait(false);
                var nestedcharsReader2 = await nestedstringDelimiterReader3.Move(context).ConfigureAwait(false);
                (context, var nestedstringDelimiterReader4) = await nestedcharsReader2.Move(context).ConfigureAwait(false);
                var nestedwhitespace6 = await nestedstringDelimiterReader4.Move(context).ConfigureAwait(false);
                //var nestedcolon2 = await nestedwhitespace6.AsReader.MoveInternal3(nestedwhitespace6.Factory).ConfigureAwait(false);
                (context, var nestedcolon2) = await nestedwhitespace6.Move(context).ConfigureAwait(false);
                var nestedwhitespace7 = await nestedcolon2.Move(context).ConfigureAwait(false);
                //var nestedvalue3 = await nestedwhitespace7.AsReader.MoveInternal3(nestedwhitespace7.Factory).ConfigureAwait(false);
                (context, var nestedvalue3) = await nestedwhitespace7.Move(context).ConfigureAwait(false);
                var nestedvalueToken3 = await nestedvalue3.Move(context).ConfigureAwait(false);
                Assert.IsTrue(nestedvalueToken3.TryFalse(out var nestedfalse));

                (context, var nestedsubsequentMembers2) = await nestedfalse.Move(context).ConfigureAwait(false);
                var nestedsubsequentMembersToken2 = await nestedsubsequentMembers2.Move(context).ConfigureAwait(false);
                Assert.IsTrue(nestedsubsequentMembersToken2.TryMore(out var nestedsubsequentMember2));

                // 1234
                var nestedcomma2 = await nestedsubsequentMember2.Move(context).ConfigureAwait(false);
                var nestedwhitespace8 = await nestedcomma2.Move(context).ConfigureAwait(false);
                //var nestedmember3 = await nestedwhitespace8.AsReader.MoveInternal3(nestedwhitespace8.Factory).ConfigureAwait(false);
                (context, var nestedmember3) = await nestedwhitespace8.Move(context).ConfigureAwait(false);
                var nestedstring3 = await nestedmember3.Move(context).ConfigureAwait(false);
                var nestedstringDelimiter5 = await nestedstring3.Move(context).ConfigureAwait(false);
                var nestedchars3 = await nestedstringDelimiter5.Move(context).ConfigureAwait(false);
                (context, var nestedstringDelimiter6) = await nestedchars3.Move(context).ConfigureAwait(false);
                var nestedwhitespace9 = await nestedstringDelimiter6.Move(context).ConfigureAwait(false);
                //var nestedcolon3 = await nestedwhitespace9.AsReader.MoveInternal3(nestedwhitespace9.Factory).ConfigureAwait(false);
                (context, var nestedcolon3) = await nestedwhitespace9.Move(context).ConfigureAwait(false);
                var nestedwhitespace10 = await nestedcolon3.Move(context).ConfigureAwait(false);
                //var nestedvalue4 = await nestedwhitespace10.AsReader.MoveInternal3(nestedwhitespace10.Factory).ConfigureAwait(false);
                (context, var nestedvalue4) = await nestedwhitespace10.Move(context).ConfigureAwait(false);
                var nestedvalueToken4 = await nestedvalue4.Move(context).ConfigureAwait(false);
                Assert.IsTrue(nestedvalueToken4.TryNumber(out var nestednumber));
                var nestedsign = await nestednumber.Move(context).ConfigureAwait(false);
                var nestedint = await nestedsign.Move(context).ConfigureAwait(false);
                (context, var nestedfrac) = await nestedint.Move(context).ConfigureAwait(false);
                var nestedfracToken = await nestedfrac.Move(context).ConfigureAwait(false);
                Assert.IsTrue(nestedfracToken.TryAbsent(out var nestedexp));
                var nestedexpToken = await nestedexp.Move(context).ConfigureAwait(false);
                Assert.IsTrue(nestedexpToken.TryAbsent(out var nestedsubsequentMembers3));
                var nestedsubsequentMembersToken3 = await nestedsubsequentMembers3.Move(context).ConfigureAwait(false);
                Assert.IsTrue(nestedsubsequentMembersToken3.TryMore(out var nestedsubsequentMember3));

                // asdf
                var nestedcomma3 = await nestedsubsequentMember3.Move(context).ConfigureAwait(false);
                ...
                (context, var nestedmember4) = await nestedwhitespace11.Move(context).ConfigureAwait(false);
                var nestedstring4 = await nestedmember4.Move(context).ConfigureAwait(false);
                var nestedstringDelimiter7 = await nestedstring4.Move(context).ConfigureAwait(false);
                var nestedchars4 = await nestedstringDelimiter7.Move(context).ConfigureAwait(false);
                (context, var nestedstringDelimiter8) = await nestedchars4.Move(context).ConfigureAwait(false);
                var nestedwhitespace12 = await nestedstringDelimiter8.Move(context).ConfigureAwait(false);
                //var nestedcolon4 = await nestedwhitespace12.AsReader.MoveInternal3(nestedwhitespace12.Factory).ConfigureAwait(false);
                (context, var nestedcolon4) = await nestedwhitespace12.Move(context).ConfigureAwait(false);
                var nestedwhitespace13 = await nestedcolon4.Move(context).ConfigureAwait(false);
                //var nestedvalue5 = await nestedwhitespace13.AsReader.MoveInternal3(nestedwhitespace13.Factory).ConfigureAwait(false);
                (context, var nestedvalue5) = await nestedwhitespace13.Move(context).ConfigureAwait(false);
                var nestedvalueToken5 = await nestedvalue5.Move(context).ConfigureAwait(false);
                Assert.IsTrue(nestedvalueToken5.TryString(out var nestedstring5));
                var nestedstringDelimiter9 = await @nestedstring5.Move(context).ConfigureAwait(false);
                var nestedchars5 = await nestedstringDelimiter9.Move(context).ConfigureAwait(false);
                (context, var nestedstringDelimiter10) = await nestedchars5.Move(context).ConfigureAwait(false);
                var nestedsubsequentMembers4 = await nestedstringDelimiter10.Move(context).ConfigureAwait(false);
                var nestedsubsequentMembersToken4 = await nestedsubsequentMembers4.Move(context).ConfigureAwait(false);
                Assert.IsTrue(nestedsubsequentMembersToken4.TryMore(out var nestedsubsequentMember4));

                // null
                var nestedcomma4 = await nestedsubsequentMember4.Move(context).ConfigureAwait(false);
                var nestedwhitespace14 = await nestedcomma4.Move(context).ConfigureAwait(false);
                //var nestedmember5 = await nestedwhitespace14.AsReader.MoveInternal3(nestedwhitespace14.Factory).ConfigureAwait(false);
                (context, var nestedmember5) = await nestedwhitespace14.Move(context).ConfigureAwait(false);
                var nestedstring6 = await nestedmember5.Move(context).ConfigureAwait(false);
                var nestedstringDelimiter11 = await nestedstring6.Move(context).ConfigureAwait(false);
                var nestedchars6 = await nestedstringDelimiter11.Move(context).ConfigureAwait(false);
                (context, var nestedstringDelimiter12) = await nestedchars6.Move(context).ConfigureAwait(false);
                var nestedwhitespace15 = await nestedstringDelimiter12.Move(context).ConfigureAwait(false);
                //var nestedcolon5 = await nestedwhitespace15.AsReader.MoveInternal3(nestedwhitespace15.Factory).ConfigureAwait(false);
                (context, var nestedcolon5) = await nestedwhitespace15.Move(context).ConfigureAwait(false);
                var nestedwhitespace16 = await nestedcolon5.Move(context).ConfigureAwait(false);
                //var nestedvalue6 = await nestedwhitespace16.AsReader.MoveInternal3(nestedwhitespace16.Factory).ConfigureAwait(false);
                (context, var nestedvalue6) = await nestedwhitespace16.Move(context).ConfigureAwait(false);
                var nestedvalueToken6 = await nestedvalue6.Move(context).ConfigureAwait(false);
                Assert.IsTrue(nestedvalueToken6.TryNull(out var nestednull));

                (context, var nestedsubsequentMembers5) = await @nestednull.Move(context).ConfigureAwait(false);
                var nestedsubsequentMembersToken5 = await nestedsubsequentMembers5.Move(context).ConfigureAwait(false);
                ...
                (context, var nestedobjectEnd) = await nestedsubsequentMember5.Move(context).ConfigureAwait(false);
                subsequentMembers6 = await nestedobjectEnd.Move(context).ConfigureAwait(false);
            }

            var subsequentMembersToken6 = await subsequentMembers6.Move(context).ConfigureAwait(false);
            Assert.IsTrue(subsequentMembersToken6.TryMore(out var subsequentMember6));

            // emptyobject
            var comma6 = await subsequentMember6.Move(context).ConfigureAwait(false);
            var whitespace21 = await comma6.Move(context).ConfigureAwait(false);
            //var member7 = await whitespace21.AsReader.MoveInternal3(whitespace21.Factory).ConfigureAwait(false);
            (context, var member7) = await whitespace21.Move(context).ConfigureAwait(false);
            var string8 = await member7.Move(context).ConfigureAwait(false);
            var stringDelimiter15 = await string8.Move(context).ConfigureAwait(false);
            var chars8 = await stringDelimiter15.Move(context).ConfigureAwait(false);
            (context, var stringDelimiter16) = await chars8.Move(context).ConfigureAwait(false);
            var whitespace22 = await stringDelimiter16.Move(context).ConfigureAwait(false);
            //var colon7 = await whitespace22.AsReader.MoveInternal3(whitespace22.Factory).ConfigureAwait(false);
            (context, var colon7) = await whitespace22.Move(context).ConfigureAwait(false);
            var whitespace23 = await colon7.Move(context).ConfigureAwait(false);
            //var value8 = await whitespace23.AsReader.MoveInternal3(whitespace23.Factory).ConfigureAwait(false);
            (context, var value8) = await whitespace23.Move(context).ConfigureAwait(false);
            var valueToken8 = await value8.Move(context).ConfigureAwait(false);
            Assert.IsTrue(valueToken8.TryObject(out var object3));
            var objectStart3 = await object3.Move(context).ConfigureAwait(false);
            var whitespace24 = await objectStart3.Move(context).ConfigureAwait(false);
            //var members3 = await whitespace24.AsReader.MoveInternal3(whitespace24.Factory).ConfigureAwait(false);
            (context, var members3) = await whitespace24.Move(context).ConfigureAwait(false);
            var membersToken3 = await members3.Move(context).ConfigureAwait(false);
            ...
            (context, var objectEnd) = await whitespace25.Move(context).ConfigureAwait(false);
            var subsequentMembers7 = await objectEnd.Move(context).ConfigureAwait(false);
            var subsequentMembersToken7 = await subsequentMembers7.Move(context).ConfigureAwait(false);
            Assert.IsTrue(subsequentMembersToken7.TryMore(out var subsequentMember8));

            // emptyarray
            var comma7 = await subsequentMember8.Move(context).ConfigureAwait(false);
            var whitespace26 = await comma7.Move(context).ConfigureAwait(false);
            //var member8 = await whitespace26.AsReader.MoveInternal3(whitespace26.Factory).ConfigureAwait(false);
            (context, var member8) = await whitespace26.Move(context).ConfigureAwait(false);
            var string9 = await member8.Move(context).ConfigureAwait(false);
            var stringDelimiter17 = await string9.Move(context).ConfigureAwait(false);
            var chars9 = await stringDelimiter17.Move(context).ConfigureAwait(false);
            (context, var stringDelimiter18) = await chars9.Move(context).ConfigureAwait(false);
            var whitespace27 = await stringDelimiter18.Move(context).ConfigureAwait(false);
            //var colon8 = await whitespace27.AsReader.MoveInternal3(whitespace27.Factory).ConfigureAwait(false);
            (context, var colon8) = await whitespace27.Move(context).ConfigureAwait(false);
            var whitespace28 = await colon8.Move(context).ConfigureAwait(false);
            //var value9 = await whitespace28.AsReader.MoveInternal3(whitespace28.Factory).ConfigureAwait(false);
            (context, var value9) = await whitespace28.Move(context).ConfigureAwait(false);
            var valueToken9 = await value9.Move(context).ConfigureAwait(false);
            Assert.IsTrue(valueToken9.TryArray(out var array));
            var arrayStart = await array.Move(context).ConfigureAwait(false);
            var whitespace29 = await arrayStart.Move(context).ConfigureAwait(false);
            //var arrayElements = await whitespace29.AsReader.MoveInternal3(whitespace29.Factory).ConfigureAwait(false);
            (context, var arrayElements) = await whitespace29.Move(context).ConfigureAwait(false);
            var arrayElementsToken = await arrayElements.Move(context).ConfigureAwait(false);
            ...
            (context, var arrayEnd) = await whitespace30.Move(context).ConfigureAwait(false);
            var subsequentMembers9 = await arrayEnd.Move(context).ConfigureAwait(false);
            var subsequentMembersToken9 = await subsequentMembers9.Move(context).ConfigureAwait(false);
            Assert.IsTrue(subsequentMembersToken9.TryMore(out var subsequentMember10));

            // array
            var comma8 = await subsequentMember10.Move(context).ConfigureAwait(false);
            var whitespace31 = await comma8.Move(context).ConfigureAwait(false);
            //var member9 = await whitespace31.AsReader.MoveInternal3(whitespace31.Factory).ConfigureAwait(false);
            (context, var member9) = await whitespace31.Move(context).ConfigureAwait(false);
            var string10 = await member9.Move(context).ConfigureAwait(false);
            var stringDelimiter19 = await string10.Move(context).ConfigureAwait(false);
            var chars10 = await stringDelimiter19.Move(context).ConfigureAwait(false);
            (context, var stringDelimiter20) = await chars10.Move(context).ConfigureAwait(false);
            var whitespace32 = await stringDelimiter20.Move(context).ConfigureAwait(false);
            //var colon9 = await whitespace32.AsReader.MoveInternal3(whitespace32.Factory).ConfigureAwait(false);
            (context, var colon9) = await whitespace32.Move(context).ConfigureAwait(false);
            var whitespace33 = await colon9.Move(context).ConfigureAwait(false);
            //var value10 = await whitespace33.AsReader.MoveInternal3(whitespace33.Factory).ConfigureAwait(false);
            (context, var value10) = await whitespace33.Move(context).ConfigureAwait(false);
            var valueToken10 = await value10.Move(context).ConfigureAwait(false);
            Assert.IsTrue(valueToken10.TryArray(out var array2));
            var arrayStart2 = await array2.Move(context).ConfigureAwait(false);
            var whitespace34 = await arrayStart2.Move(context).ConfigureAwait(false);
            //var arrayElements2 = await whitespace34.AsReader.MoveInternal3(whitespace34.Factory).ConfigureAwait(false);
            (context, var arrayElements2) = await whitespace34.Move(context).ConfigureAwait(false);
            var arrayElementsToken2 = await arrayElements2.Move(context).ConfigureAwait(false);
            Assert.IsTrue(arrayElementsToken2.TrySome(out var arrayElement));
            var value11 = await arrayElement.Move(context).ConfigureAwait(false);
            var valueToken11 = await value11.Move(context).ConfigureAwait(false);
            Assert.IsTrue(valueToken11.TryObject(out var object4));
            var objectStart4 = await object4.Move(context).ConfigureAwait(false);
            var whitespace35 = await objectStart4.Move(context).ConfigureAwait(false);
            //var members4 = await whitespace35.AsReader.MoveInternal3(whitespace35.Factory).ConfigureAwait(false);
            (context, var members4) = await whitespace35.Move(context).ConfigureAwait(false);
            var membersToken4 = await members4.Move(context).ConfigureAwait(false);
            Assert.IsTrue(membersToken4.TrySome(out var firstMember4));
            Json5.SubsequentArrayElementsReader<Json5.WhitespaceReader<Json5.ArrayEndReader<Json5.SubsequentMembersReader<Json5.WhitespaceReader<Json5.ObjectEndReader<Json5.WhitespaceReader<Nothing>>>>>>> subsequentArrayElements;
            {
                // true
                var nestedMember = await firstMember4.Move(context).ConfigureAwait(false);
                var nestedString = await nestedMember.Move(context).ConfigureAwait(false);
                var nestedStringDelimiter = await nestedString.Move(context).ConfigureAwait(false);
                var nestedChars = await nestedStringDelimiter.Move(context).ConfigureAwait(false);
                (context, var nestedStringDeliminter2) = await nestedChars.Move(context).ConfigureAwait(false);
                ...
                //var nestedobjectEnd = await nestedsubsequentMember5.AsReader.MoveInternal3(nestedsubsequentMember5.Factory).ConfigureAwait(false);
                (context, var nestedobjectEnd) = await nestedsubsequentMember5.Move(context).ConfigureAwait(false);
                subsequentArrayElements = await nestedobjectEnd.Move(context).ConfigureAwait(false);
            }

            var subsequentArrayElementsToken = await subsequentArrayElements.Move(context).ConfigureAwait(false);
            ...
            (context, var arrayEnd2) = await whitespace36.Move(context).ConfigureAwait(false);
            var subsequentMembers10 = await arrayEnd2.Move(context).ConfigureAwait(false);
            var subsequentMembersToken10 = await subsequentMembers10.Move(context).ConfigureAwait(false);
            ...
            (context, var objectEnd2) = await whitespace37.Move(context).ConfigureAwait(false);
            var whitespace38 = await objectEnd2.Move(context).ConfigureAwait(false);
            //// TODO remove this when you fix moveinternal3
            ////context.Read().ConfigureAwait(false).GetAwaiter().GetResult();
            (context, var nothing) = await whitespace38.Move(context).ConfigureAwait(false);
