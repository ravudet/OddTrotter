        ...
        private sealed class CalendarEventsSource : OddTrotter.ICalendarEventsSource
        {
            ...
            private sealed class CalendarEventsContext : OddTrotter.ICalendarEventsContext
            {
                ...
                public async ITask<IQueryResult<IEither<OddTrotter.CalendarEvent, OddTrotter.CalendarEventTranslationError>, OddTrotter.PagingError>> Evaluate()
                {
                    ...
                    //// TODO i don't like about the above because these files no longer follow the "the code looks like the csdl" principle because the csdl just says its a collection, its the standard that says "oh, and by the way, collections are called in some specific way" //// TODO that said, does the extension provide enough support to overcome this? because, honestly, as an implementer of the client, having to do the concatenation is a bit janky //// TODO maybe looking at a service implementation will help? //// TODO good point, from the service implementer perspective, they need to know to do paging at all, so really the api on the interface should have a contract that forces the service implementer to consider the paging, even if they say "no paging, here's the entire collection" //// that would be pretty straightforward if the return type of this method were strongly typed and had several control information properties; and with that in mind, how is something like `$count` being exposed right now?
                    ...
                    //// TODO i think that all of this basically puts you back at the strong convention context layer //// TODO but actually it doesn't because of the URL building; so you still have a layer on top of strong convention context, and it's the layer that has a fluent api for building the url (even though the return types are the same as the strong convention context) //// TODO i haven't actually fleshed out (even in my head) what the contracts of this layer would be, so maybe it doesn't work
                    //// TODO such a layer still implies that there is a "concrete type" layer specific to a given service (e.g. having a `calendareventcontext : inewlayercontext<graph.calendarevent>`)
                    //// TODO you need to also have a way to add headers if you abstracting away the url stuff; *maybe* that should be in its own layer (because you are putting *every* little thing in separate layers), but regardless, the "top" layer should have this accounted for (and note that, like control information, you need headers in the return value too)
                    ...
                    out Expression<Func<Graph.CalendarEvent, bool>>? translatedFilter)
                {
                    ...
                    var calendarEventParameter = currentFilter.Parameters[0];

                    var translatedcalendarEventParameter = Expression.Parameter(typeof(Graph.CalendarEvent), calendarEventParameter.Name);
                    var visitor = new ExpressionVisitor(calendarEventParameter, translatedcalendarEventParameter);
                    var translatedBody = visitor.Visit(currentFilter.Body);

                    translatedFilter = calendarEvent => true;
                    translatedFilter = translatedFilter.Update(translatedBody, new[] { translatedFilter.Parameters[0] }); //// new[] { translatedcalendarEventParameter });

                    var lambda = Expression.Lambda<Func<Graph.CalendarEvent, bool>>(translatedBody, translatedcalendarEventParameter);
                    translatedFilter = lambda;

                    translatedFilter.Compile(); // you return the non-compiled version, but you compile it anyway so that you get error handling
                }
                ...
                private sealed class ExpressionVisitor : System.Linq.Expressions.ExpressionVisitor
                {
                    ...
                    public bool ApplicableToSeriesMaster { get; private set; } //// TODO you're not actually using this anywhere at the moment; doesn't mean that you should be, but it might help make some things more general (so you don't have to use the static expressions in the caller)
                    ...
                    private static Expression<Func<Graph.CalendarEvent, DateTime>> StartExpression { get; } = calendarEvent => calendarEvent.Start.DateTime;
                    private static Expression<Func<Graph.CalendarEvent, DateTime>> EndExpression { get; } = calendarEvent => calendarEvent.End.DateTime;
